...
class PerceptronModel(object):
	...
	def train(self, dataset):
		...
		while True:
			issues = 0
			#loop through and check each node/update
	...
	def train(self, dataset):
		...
		while loss > .02:
			...
			for x, y in dataset.iterate_once(1):
				...
				tempLoss = self.get_loss(x,y)
				fullLoss+= nn.as_scalar(tempLoss)
				#grad_wrt_m, grad_wrt_b = nn.gradients(loss, [m, b])
				gm1,gm2,gb1,gb2 = nn.gradients(tempLoss, [self.m1, self.m2, self.bias1, self.bias2])

				#m.update(grad_wrt_m, multiplier)
				#1,-1,-.1,.1,.05,.001,-.05
				#must be negative, or tiny positive
				#
				multiplier = -.01
				self.m1.update(gm1, multiplier)
